...
#define LoRa_ModeSettingPin_M0 18
#define LoRa_ModeSettingPin_M1 5
#define LoRa_RxPin 27
#define LoRa_TxPin 26
#define LoRa_AUXPin 4
...
private:
  uint32_t DetectLoRaBaudRate(void);
  void ReadLoRaResponse(size_t expected_size, uint32_t timeout_ms, std::vector<uint8_t> &response);
  bool WaitForAuxHigh(uint32_t timeout_ms);
  void DrainLoRaInput(void);
