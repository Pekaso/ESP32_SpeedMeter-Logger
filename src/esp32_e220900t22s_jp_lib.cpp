...
int CLoRa::InitLoRaModule(struct LoRaConfigItem_t &config) {
  ...
  SerialMon.printf("LoRa UART begin: baud=%d rx=%d tx=%d aux=%d\n",
                   LoRa_BaudRate, LoRa_RxPin, LoRa_TxPin, LoRa_AUXPin);
  SerialMon.printf("AUX before command: %d\n", digitalRead(LoRa_AUXPin));
  if (!WaitForAuxHigh(1000)) {
    SerialMon.printf("AUX did not become HIGH before configuration command\n");
    ret = 1;
  }
  DrainLoRaInput();
...
// When do not use Semaphore... Sample
//  for (auto i : command) {
//    SerialLoRa.write(i);
//  }
//  SerialLoRa.flush();
...
//  }
  ...
  if (xSemaphoreTake(xMutex, (portTickType)100) == pdTRUE) {
    ...
    SerialMon.printf("failed to lock LoRa UART for configuration command\n");
    ret = 1;
  }

  if (!WaitForAuxHigh(1000)) {
    SerialMon.printf("AUX did not become HIGH after configuration command\n");
    ret = 1;
  }

  uint32_t started_at = millis();
  while ((response.size() < command.size()) && ((millis() - started_at) < 1000)) {
    while (SerialLoRa.available()) {
      if (xSemaphoreTake(xMutex, (portTickType)100) == pdTRUE) {
        ...
        SerialMon.printf("failed to lock LoRa UART while reading response\n");
        ret = 1;
        break;
      }
    }
  ...
  if (response.size() != command.size()) {
    SerialMon.printf("unexpected response size: expected=%u actual=%u\n",
                     (unsigned int)command.size(), (unsigned int)response.size());
    ret = 1;
  }
...
bool CLoRa::WaitForAuxHigh(uint32_t timeout_ms) {
  uint32_t started_at = millis();
...
void CLoRa::DrainLoRaInput(void) {
  while (SerialLoRa.available()) {
    SerialLoRa.read();
  }
}
